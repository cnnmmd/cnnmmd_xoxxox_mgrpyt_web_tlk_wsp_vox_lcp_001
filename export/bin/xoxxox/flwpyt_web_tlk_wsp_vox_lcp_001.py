...
async def tlkweb():
  ...
  datwav = await MidClt.reqprc({"keymmd": datweb["keymmd"], "keyprc": "xoxxox.CnvVce.webwav"}, adrmid + MidClt.adrprc) # 音声変換（web -> wav）
  ...
  datres = await MidClt.reqprc({"keymmd": dattts["keymmd"]}, adrmid + MidClt.adrgps + "000") # 待機（出力）→音声（ウェブブラウザ）
